...
const Amenities = (props) => {
  ...
  const amenities = { amenities: selectedAmenities }

  const fetchAmenitiesApi = async () => {
    try {
  ...
  return (
    <main className='lg:p-10 lg:h-auto md:p-10 sm:py-10 sm:px-5 '>
      <h1 className='text-[18px] font-normal py-2'>
        {t('Let your guest know what amenities you have')}
      </h1>
      <p className='font-medium text-[20px] '> 7/11 </p>
      ...
      <section className='lg:my-4'>
        <Formik>
          {({ values }) => (
            <Form>
              <div
                className='lg:grid lg:grid-cols-2 lg:gap-5'
                role='group'
                aria-labelledby='amenities-group'
              >
                {amenitiesData?.map((options) => (
                  <div
                    key={options.id}
                    className='flex flex-col gap-3 lg:py-1 md:py-1 sm:py-3'
                  >
                    <h4 className='font-medium'> {options.name} </h4>
                      ...
                      return (
                        ...
                        >
                          <input
                            ...
                            id=''
                            onChange={(e) =>
                              amenitiesCalc(e.target.checked, items)
                            }
                          />
                          {items.name}
                        </label>
              ...
              <button
                type='button'
                onClick={() => props.next(amenities)}
                className='border-0 text-center rounded-md hover:bg-[#0559FD] bg-[#D4D4D4] text-white w-[80px] h-[50px] lg:mt-5
                md:mt-20 sm:mt-10 '
              >
                {t('Next')}
              </button>
              ...
              <section
                className='progressbar lg:flex lg:item-center lg:justify-center lg:gap-4 lg:pt-10 lg:py-0 md:pt-24
                md:flex md:justify-between md:items-center md:gap-4 md:py-0 sm:flex sm:justify-between sm:items-center
                sm:gap-4 sm:pt-14 sm:py-5 '
              >
                <div className='lg:w-[90%] border-b-2 border-[#F2F2F2] relative top-0 md:w-[90%] sm:w-[90%] '>
                  <div className='w-[50%] border-b-2 border-blue-500 absolute'></div>
                </div>
