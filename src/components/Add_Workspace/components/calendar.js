...
const AvailableBookings = (props) => {
  ...
  const bookings = { bookings: bookingDate }
  ...
  return (
    <main className='lg:mt-10 lg:px-10 md:mt-10 md:px-10 sm:mt-5 sm:px-5 '>
      <h1 className='text-2xl font-normal py-1'>
        {t('Let’s set up your availability')}
      </h1>
      <p className='font-sembold text-[20px] '>
        {' '}
        10/{props?.data['type'] == 'Hourly' ? '12' : '11'}{' '}
      </p>

      <div className='py-10 relative'>
        <p> {t('Tap to create booking')} </p>
        ...
        {/* {timeModal && (
          <div
            className='absolute top-[300px] left-[200px] w-[300px] h-[176px] rounded-lg bg-white shadow-2fl 
            grid grid-cols-1 z-10 px-5'
          >
            ...
            <div className='flex flex-row gap-3 ml-auto'>
              <button
                className='border-2 rounded-md w-[90px] h-[40px] bg-white '
              ...
              <button
                className='bg-[#0559FD] rounded-md w-[90px] h-[40px] text-white '
                onClick={() => setBookDate()}
              >
                {' '}
                Reschedule{' '}
      ...
      <section
        className='progressbar lg:flex lg:item-center lg:justify-center lg:gap-4 lg:pt-0 lg:py-6 md:pt-24
      ...
      >
        <div className=' lg:w-[90%] border-b-2 border-[#F2F2F2] relative top-0 md:w-[90%] sm:w-[90%] '>
          <div className='w-[80%] border-b-2 border-blue-500 absolute'></div>
        </div>

        <div className='btns flex space-x-4'>
          <button
            type='button'
            onClick={() => props.prev(bookings)}
