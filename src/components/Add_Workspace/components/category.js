...
const Category = (props) => {
  ...
  const fetchCategories = async () => {
    ...
    if (data?.status) {
      set_categories_avaliable((prev) => [...prev, ...data?.data])
    }
  }
  ...
  const displayOptions = categories_avaliable?.map((x, i) => {
    return <option value={x?.id}>{x?.name}</option>
  })

  const handleNext = () => {
    if (!categorySelected) {
      setDisplayError('Please choose a category to proceed')
      return
    }

    let values = { ...props.data, category: parseInt(categorySelected) }
    props.next(values)
  }
  ...
  return (
    <main className='lg:p-10 lg:h-screen md:p-10 sm:p-10'>
      <h1 className='text-2xl font-normal py-1'>
        {t('Lets start with the category of your space')}
      </h1>
      <p className='font-semibold text-[20px] '> 4/11 </p>
      ...
      >
        {({ values }) => (
          <Form className='grid grid-cols-1 content-center lg:my-32 md:mt-20 sm:mt-20'>
            ...
            <div className='text-[red]'>{displayError}</div>
            <button
              type='submit'
