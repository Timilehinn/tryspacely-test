...
const Add_Description = (props, data) => {
  ...
  const onEditorStateChange = (editor) => {
    ...
    const editorText = draftToHtml(convertToRaw(editor.getCurrentContent()))
    setDescription(editorText)
    props.data['description'] = editorText
  }

  return (
    <main className='lg:p-10 lg:h-screen md:p-10 sm:p-10'>
      <h1 className='text-[20px] font-normal py-1'>
        {t('A little description about your space')}
      </h1>
      <p className='font-semibold text-[20px]'> 2/11 </p>

      <div>
        <div className='flex flex-col'>
          <label className='font-medium py-2'> {t('Description')} </label>
        </div>
        <div className='pb-[3rem]'>
          <Editor
            ...
            className='outline-0 border-[2px] w-full lg:h-[180px]  border-red rounded-lg md:h-[180px] sm:h-[180px] p-2'
          />
        </div>
        <button
          type='button'
          onClick={() => props.next(props.data)}
          className='border-0 rounded hover:bg-[#0559FD] bg-[#D4D4D4] text-white w-[80px] h-[52px] lg:mt-5
                md:mt-20 sm:mt-10 text-center'
        >
          {t('Next')}
        </button>

        <section
          className='progressbar lg:flex lg:item-center lg:justify-center lg:gap-4 lg:pt-20 md:pt-32
        ...
        >
          <div className=' lg:w-[90%] border-b-2 border-[#F2F2F2] relative top-0 md:w-[90%] sm:w-[90%] '>
            <div className='w-[10%] border-b-2 border-blue-500 absolute'></div>
          </div>
