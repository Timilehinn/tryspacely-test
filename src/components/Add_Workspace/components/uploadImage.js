...
const UploadImage = (props) => {
  ...
  const onImageChange = (e) => {
    ...
    Object.keys(files).forEach((key) => {
      ...
      reader.onloadend = () => {
        setFiles(prev => [...prev, reader.result.toString()]);
      }
  ...
  return (
    <main className='lg:p-10 md:p-10 sm:p-5'>
      <h1 className='text-2xl font-normal py-1 lg:pt-0 md:pt-0 sm:pt-10'>
        {t("Give your guest a little insight on what to expect")}
      </h1>
      <p className='font-semibold text-[20px] '> 8/11 </p>

      <div className='lg:w-full lg:py-5 md:py-7 sm:py-5 '>
        <div className='w-full xl:h-[450px] lg:h-[450px] md:h-[300px] sm:h-[300px] my-5 rounded-md'>
          {files >= 0 ? (
            <div className="flex flex-col gap-2 justify-center items-center bg-white shadow-2fl lg:h-[250px] md:h-[300px] sm:h-[250px] ">
              <span className="text-2xl font-medium"> Add photos </span>
              <span className="font-normal py-2"> Add at least 5 photos </span>
              <input
                type='file'
                multiple
                accept='image/*'
              ...
              <label
                htmlFor='file'
                className='border-2 border-[#E5E5E5] py-2 px-5 rounded-md cursor-pointer '
              >
                {t("Upload")}
              </label>
            </div>
            ...
            <div className="xl:grid xl:grid-cols-2 lg:h-[400px] lg:grid lg:grid-cols-2 md:grid md:grid-cols-2 md:h-[300px] 
            sm:h-[300px] sm:grid sm:grid-cols-1 gap-2  overflow-auto ">
              {files.map((img, key) => (
                <img
                  src={img}
                  key={key}
        ...
        >
          <div className=' lg:w-[90%] border-b-2 border-[#F2F2F2] relative top-0 md:w-[90%] sm:w-[90%] '>
            <div className='w-[60%] border-b-2 border-blue-500 absolute'></div>
          </div>

          <div className='btns flex space-x-4'>
            <button
              type='button'
              onClick={() => props.prev(photos)}
