...
const AddUser = (props) => {
  ...
  const users = { available_space: count }
  ...
  return (
    <main className='lg:p-10 lg:h-auto md:p-10 sm:p-5'>
      <h1 className='text-[20px] font-normal py-1'>
        {t("How many users can your space accomodate")}
      </h1>
      <p className='font-semibold text-[20px] '> 6/11 </p>

      <div className='grid grid-cols-1 content-center lg:mt-20 md:mt-32 sm:mt-20'>
        <section
          className='lg:flex lg:flex-row lg:gap-10 lg:justify-center lg:items-center md:flex md:flex-row
          md:gap-10 md:justify-center md:item-center sm:flex sm:flex-row sm:gap-5 sm:justify-center sm:items-center'
        >
          <button
            onClick={decrement}
            className='border-2 border-solid border-[#AAAAAA] rounded-full w-[36px] h-[36px] flex justify-center items-center '
          >
            <svg
              width='20'
              height='10'
          ...
          <button
            onClick={increment}
            className='border-2 border-solid border-[#AAAAAA] rounded-full w-[36px] h-[36px]
        ...
        <h1 className='text-center text-2xl py-4 font-medium text-[#727073]'>
          {' '}
          Users{' '}
        </h1>
      </div>
      ...
      <button
        type='submit'
        onClick={() => props.next(users)}
        className='border-0 rounded hover:bg-[#0559FD] bg-[#D4D4D4] text-white w-[80px] h-[52px] lg:mt-10
        md:mt-32 sm:mt-20 text-center'
      >
        Next
      </button>

      <section
        className='progressbar lg:flex lg:item-center lg:justify-center lg:gap-4 lg:pt-20 md:pt-28
      ...
      >
        <div className=' lg:w-[90%] border-b-2 border-[#F2F2F2] relative top-0 md:w-[90%] sm:w-[90%] '>
          <div className='w-[40%] border-b-2 border-blue-500 absolute'></div>
        </div>

        <div className='btns flex space-x-4'>
          <button
            type='button'
            onClick={() => props.prev(users, props?.data?.category === 2 ? 'allowPrev' : 'skipPrev')}
